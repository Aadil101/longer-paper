...
\begin{figure}
 ...
 \caption{\label{fig:head_3-9_tf_avg} Here, we consider three arbitrary test set samples: 279, 334, and 244. For each sample, we compute the word frequency of each word in the context, relative to the GBND corpus. For simplicity, we let the word frequency of a BPE be identical to the word frequency of the original word it was derived from. We then compute the average attention weight directed to each word in the context at head 3-9 (ie. layer 3, head 9 in Figure \ref{fig:head_correlations_tf}).}
\end{figure}
...
\begin{figure}
  ...
  \end{subfigure}\vspace{10mm}
  \caption{\label{fig:head_3-9} Here, we once again study test set samples 279, 334, and 244. For each sample, we visualize the head 3-9 attention weight directed between any two BPEs in the context.}
\end{figure}
...
Based on our prior findings regarding the importance of term frequency-based features to the performance of the XGBoost$_\textit{full}$ model, we hypothesize that at certain attention heads, the average attention given to BPEs varies relative to the parent word's rarity in lexicon. This follows the findings of \citealp{voita2019analyzing}, who identify heads at which lesser frequent tokens are attended to semi-uniformly by a majority of sentence tokens. 

To test our hypothesis, we estimate for each attention head the Pearson correlation between word frequency and the average attention weight directed to each word in the context.\footnote{We compute attention given to a \textit{word} as the sum of attention weights given to its constituent BPEs. We use the GBND corpus to extract word frequencies from, though any large corpora would suffice.} As illustrated in Figures \ref{fig:head_correlations_tf}, we find multiple attention heads appearing to specialize at directing attention towards either the most or least (depending on sign of the correlation) frequent words in language. Vertical stripe patterns like that in Figure \ref{fig:head_3-9} emerge as a result of attention originating from a spectrum of tokens; note that any cell $(i,j)$ in an attention map represents the attention weight directed \textit{from} the $i$th token \textit{to} the $j$th token in the sentence. A shortcoming of this examination is that the specific roles played by tokens responsible for the vertical stripe patterns over particular tokens remains unclear. Nonetheless, these findings seem to affirm the fundamental relevancy of word frequency to lexical complexity prediction, corroborating our intuitions.
